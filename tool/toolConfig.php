...
class toolConfig{
	...
	*/
	function toolConfig( $path=null, $synchronize=false, $process_sections=true){
		...
		if ( $path!=null) {
			...
			if ( !is_file( $path)) {
				...
				if ( !$fp_new) {
					$err = "Config::Config() - Could not create new config-file('$path'), error.";
	...
	*/
	...
	function removeKey( $key, $section=null){
		...
		if ( $section!=null){
			if ( in_array( $section, array_keys( $this->VARS))==false){
				$err = "Config::removeKey() - Could not find section('$section'), nothing was removed.";
				array_push( $this->ERRORS, $err);
				return false;
			}
			// look if given key exists in given section
			if ( in_array( $key, array_keys( $this->VARS[$section]))===false) {
				$err = "Config::removeKey() - Could not find key('$key'), nothing was removed.";
				array_push( $this->ERRORS, $err);
				return false;
			}
	...
	function removeSection( $section){
		...
		if ( in_array( $section, array_keys( $this->VARS), true)===false) {
			$err = "Config::removeSection() - Section('$section') could not be found, nothing removed.";
			array_push( $this->ERRORS, $err);
			return false;
		}
	...
	*/
	function load( $path=null){
		...
		if ( $path!=null and !is_file( $path)) {
			$err = "Config::load() - Path('$path') is invalid, nothing loaded.";
			array_push( $this->ERRORS, $err);
			echo $err;
			return false;
		}elseif ( $path==null){
	...
	*/
	function save( $path=null){
		...
		if ( !fwrite( $handle, $content)) {
			$err = "Config::save() - Could not write to open file('$path'), error.";
			array_push( $this->ERRORS, $err);
			return false;
		}else{
			// push a message onto error-stack
			$err = "Config::save() - Sucessfully saved to file('$path').";
			array_push( $this->ERRORS, $err);
		}
		fclose( $handle);
		return true;
	...
	function toString( $output_type='TEXT'){
		...
		if ( strtoupper( $output_type)!=='TEXT' and strtoupper( $output_type)!=='HTML') {
			$err = "Config::toString() - Unknown OutputType('$output_type') was requested, falling back to TEXT.";
			array_push( $this->ERRORS, $err);
			$output_type = 'TEXT';
		}
    ...
    function getSection($sectionName){
        if( (array_key_exists($sectionName, $this->VARS)) and (is_array($this->VARS[$sectionName])) ){
            $section = $this->VARS[$sectionName];
